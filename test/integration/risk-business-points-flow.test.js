...
jest.mock('wx-server-sdk', () => {
  ...
  const collection = name => {
    ...
    return {
      ...
      add: async ({ data: doc }) => {
        if (name === 'orders') {orders.push(doc)}
        if (name === 'activity_registrations') {registrations.push(doc)}
        return { _id: doc._id || `mock_id_${Math.random().toString(36).slice(2, 8)}` }
      },
    }
  }
...
describe('Sprint 22: RATE_LIMITED 透传', () => {
  test('多次调用触发限流后应被拦截', async () => {
    // 把内存 store 全局塞满
    const { setGlobalRateLimitStore, _resetStore: resetStore } = require(path.join(COMMON_DIR, 'risk-rate-limit.js'))
    resetStore()
    setGlobalRateLimitStore(null)
