...
describe('Sprint 24: paymentService/refund TypeScript 迁移', () => {
  ...
  describe('7. package.json 脚本注册', () => {
    ...
    test('注册 build:payment-service', () => {
      expect(pkg.scripts['build:payment-service']).toBe('node scripts/build-payment-service.js')
    })
    ...
    test('build:all 包含 build:payment-service', () => {
      expect(pkg.scripts['build:all']).toMatch(/build:payment-service/)
    })
  ...
  describe('8. scripts/build-payment-service.js 存在', () => {
    test('build-payment-service.js 脚本应存在', () => {
      expect(fs.existsSync(path.join(ROOT, 'scripts', 'build-payment-service.js'))).toBe(true)
    })

    test('脚本包含 refund.js 作为构建目标', () => {
      const code = fs.readFileSync(
        path.join(ROOT, 'scripts', 'build-payment-service.js'),
        'utf8'
      )
      expect(code).toMatch(/refund\.js/)
