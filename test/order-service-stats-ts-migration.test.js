...
describe('Sprint 30: orderService/stats TypeScript 迁移', () => {
  ...
  describe('2. tsconfig.orderService.json 配置', () => {
    ...
    test('include 仍包含 payment.ts（Sprint 29 回归）', () => {
  ...
  describe('12. orderService TypeScript 迁移完成度', () => {
    ...
    test('payment.ts 已迁移（Sprint 29）', () => {
      expect(fs.existsSync(path.join(STATS_DIR, 'payment.ts'))).toBe(true)
    })
    ...
    test('orderService 3/3 .ts 文件全部存在', () => {
      const tsFiles = ['orders.ts', 'payment.ts', 'stats.ts']
