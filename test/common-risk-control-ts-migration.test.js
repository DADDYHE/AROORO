...
describe('Sprint 15: risk-control.ts TypeScript 迁移', () => {
  ...
  describe('.js 产物：行为接口一致', () => {
    ...
    test('assertRiskDecision 行为正确', () => {
      ...
      // reject
      try { risk.assertRiskDecision(baseRisk('reject')); throw new Error('should throw') }
      catch (e) { expect(e.code).toBe('RISK_REJECT') }
      // review
      try { risk.assertRiskDecision(baseRisk('review')); throw new Error('should throw') }
      catch (e) { expect(e.code).toBe('RISK_PENDING') }
    })
  })
  ...
  describe('tsconfig / build 工具链', () => {
    ...
    test('scripts/build-common.js 包含 risk-control.js', () => {
