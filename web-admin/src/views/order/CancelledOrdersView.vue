...
async function fetchOrders(params) {
  if (orderType.value === 'all') {
    const results = await Promise.all(ALL_TYPES.map(type =>
      fetchFns[type]({ page: params.page, pageSize: params.pageSize, status: 'cancelled', ...params })
  ...
  const result = await fetchFns[orderType.value]({ page: params.page, pageSize: params.pageSize, status: 'cancelled', ...params })
  const rawList = result.data?.list || result.data || []
  const filteredList = rawList.filter(item => item.status === 'cancelled')
  return { data: { list: filteredList, total: result.data?.total || 0 } }
}
...
async function onExport() {
  try {
    ...
    link.download = `取消订单_${new Date().toISOString().slice(0, 10)}.csv`
    link.click()
