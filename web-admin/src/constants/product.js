export const PRODUCT_STATUS = {
  on_sale: { label: '在售', tagType: 'success' },
...
export const CATEGORY_CASCADER_OPTIONS = MALL_CATEGORIES.map(cat => ({
  value: cat.key,
  label: cat.label,
...
export function getCategoryLabel(category, categoryId) {
  const cat = MALL_CATEGORIES.find(c => c.key === category)
  if (!cat) {return category || ''}
  if (!categoryId) {return cat.label}
  const sub = cat.subcats.find(s => s.key === categoryId)
  return sub ? `${cat.label} / ${sub.label}` : cat.label
}

export function formatPrice(row) {
  if (row.skuType === 'multi' && row.minPrice !== undefined) {
    const min = Number(row.minPrice).toFixed(2)
    const max = Number(row.maxPrice).toFixed(2)
