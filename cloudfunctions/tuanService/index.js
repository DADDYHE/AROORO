    ...
    if (order.tuanOrderId) {
        try {
            ...
            try {
                ...
                await recordAlert('warning', 'tuan.ship.syncTuanOrder.failed', '发货后 tuan_orders 状态同步失败', { orderId, tuanOrderId: order.tuanOrderId, targetStatus: 'pending_shipment', error: e.message });
            }
            catch { /* best-effort */ }
            await recordFailedOperation('sync_tuan_order_status', { orderId, tuanOrderId: order.tuanOrderId, targetStatus: 'pending_shipment' }, e);
        }
    }
    // L1: 写操作审计日志（best-effort）
    await writeOperationLog({
