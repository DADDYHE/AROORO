...
async function restoreTuanDealStock(dealId, productId, skuId, quantity) {
    try {
        ...
        logger.warn('cancelTuanOrder.restoreTuanDealStock', { dealId, productId, msg: e?.message });
    }
}
/** 取消订单时回退 tuan_deals 累计单数/金额（与下单 inc 对称，防止列表统计虚高） */
...
async function writeOperationLog(params) {
    try {
        ...
        logger.warn('writeOperationLog.failed', { msg: e?.message });
    }
}
// =====================================================================
// Action 1：拉取团购列表
...
async function getTuanDealDetail(event) {
    ...
    try {
        ...
        for (const p of (deal.products || [])) {
            if (p.skuType === 'multi' && p.skus && p.skus.length > 0) {
                ...
                for (const sku of p.skus) {
                    if (sku.enabled !== false) {
                        const price = Number(sku.tuanPrice) || Number(sku.price) || Infinity;
                        if (price < (p.minSkuPrice ?? Infinity)) {
                            p.minSkuPrice = price;
                        }
                    }
                }
        ...
        logger.error('getTuanDealDetail', { targetId, msg: error?.message });
...
async function createTuanOrder(event, _context, auth) {
    ...
    // M2: 下单风控前置（限流 + 反刷单）
    try {
        await withRateLimit({ userId: openid, type: 'tuan_order', targetId: dealId }, async () => {
        ...
        logger.warn('createTuanOrder.rateLimit', { openid, dealId, msg: e?.message });
    ...
    const grossAmount = finalPrice * quantity;
    ...
    let couponSnapshot;
    if (couponId) {
        const lockResult = await validateAndLockCoupon(openid, couponId, grossAmount, '', 'tuan');
    ...
    if (couponId && finalAmount < 0.1) {
        ...
        if (couponId) {
            await unlockCouponBestEffort(couponId, '');
        }
    ...
    const idempotencyKey = `tuan_${openid}_${dealId}_${productId}_${skuId || ''}`;
    ...
    const order = {
        ...
        originalAmount: originalAmount || grossAmount,
    ...
    try {
        ...
        const freshDealRes = await transaction.collection('tuan_deals').doc(dealId).get();
        ...
        if (skuId && freshProduct.skuType === 'multi' && freshProduct.skus) {
            ...
            if (skuIndex >= 0) {
                ...
                const stockField = (sku.tuanStock !== undefined && sku.tuanStock !== null) ? 'tuanStock' : 'stock';
                updateData[`products.${freshProductIndex}.skus.${skuIndex}.${stockField}`] = _.inc(-quantity);
        ...
        await transaction.collection('tuan_deals').doc(dealId).update({ data: updateData });
        ...
        }).catch(e => logger.warn('createTuanOrder.auditLog', { msg: e?.message }));
        ...
        if (couponId) {
            await unlockCouponBestEffort(couponId, '').catch(() => { });
        }
        throw error;
    }
}
...
async function shipTuanOrder(event, _context, auth) {
    ...
    if (order.tuanOrderId) {
        try {
            ...
            try {
                ...
                await recordAlert('warning', 'tuan.ship.syncTuanOrder.failed', '发货后 tuan_orders 状态同步失败', { orderId, tuanOrderId: order.tuanOrderId, targetStatus: 'shipped', error: e.message });
            }
            catch { /* best-effort */ }
            await recordFailedOperation('sync_tuan_order_status', { orderId, tuanOrderId: order.tuanOrderId, targetStatus: 'shipped' }, e);
        }
    }
    ...
    if (transactionId) {
        try {
            ...
            logger.warn('shipTuanOrder.uploadShippingInfo.exception', {
                orderId, msg: e?.message || String(e),
            });
        }
                    ...
                    if (order.tuanOrderId) {
                        try {
                            ...
                            logger.warn('shipTuanOrder.syncWaybillTokenToTuanOrder.failed', {
                                orderId, tuanOrderId: order.tuanOrderId, msg: e?.message,
                            });
                        }
                    }
                }
    ...
    // L1: 写操作审计日志（best-effort）
    await writeOperationLog({
        module: 'tuan_order', action: 'ship', targetId: orderId,
        operatorId: auth.adminId || auth.openid,
        afterData: { status: 'shipped', expressCompany, expressNo },
    }).catch(e => logger.warn('shipTuanOrder.auditLog', { msg: e?.message }));
...
async function confirmReceiveTuanOrder(event, _context, auth) {
    ...
    if (order.tuanOrderId) {
        try {
            ...
            try {
                const { recordAlert } = require('./common/alert');
                await recordAlert('warning', 'tuan.confirm.syncTuanOrder.failed', '确认收货后 tuan_orders 状态同步失败', { orderId, tuanOrderId: order.tuanOrderId, targetStatus: 'completed', error: e.message });
            }
            catch { /* best-effort */ }
            await recordFailedOperation('sync_tuan_order_status', { orderId, tuanOrderId: order.tuanOrderId, targetStatus: 'completed' }, e);
        }
    }
    // L1: 写操作审计日志（best-effort）
    await writeOperationLog({
        module: 'tuan_order', action: 'confirm', targetId: orderId,
        operatorId: openid,
        afterData: { status: 'completed' },
    }).catch(e => logger.warn('confirmReceiveTuanOrder.auditLog', { msg: e?.message }));
...
async function cancelTuanOrder(event, _context, auth) {
    ...
    if (['paid', 'pending_shipment'].includes(order.status)) {
        try {
            ...
            logger.error('cancelTuanOrder.refundFailed', { orderId, msg: e.message });
            try {
    ...
    try {
        await restoreTuanDealStock(order.dealId, order.productId, order.skuId || '', Number(order.quantity) || 1);
    }
    catch (stockErr) {
        ...
        await recordFailedOperation('restore_tuan_deal_stock', { orderId, dealId: order.dealId, productId: order.productId, skuId: order.skuId, quantity: order.quantity }, stockErr);
    }
    ...
    try {
        ...
        logger.warn('cancelTuanOrder.rollbackTotals.failed', { orderId, dealId: order.dealId, msg: totalsErr?.message });
        await recordFailedOperation('rollback_tuan_deal_totals', { orderId, dealId: order.dealId, totalAmount: order.totalAmount }, totalsErr);
    }
    // L1: 写操作审计日志（best-effort）
    await writeOperationLog({
