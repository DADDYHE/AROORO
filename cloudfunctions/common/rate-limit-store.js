...
async function consumeGlobalRateLimit(input, store) {
    ...
    try {
        ...
        throw (0, errors_1.err)('INTERNAL_ERROR', `全局限流写入失败：${(e && e.message) || 'unknown'}`);
    }
    // 5. 写入 target（如有）
    if (input.targetId && targetKey) {
        ...
        try {
            ...
            throw (0, errors_1.err)('INTERNAL_ERROR', `目标限流写入失败：${(e && e.message) || 'unknown'}`);
        }
    }
    // 6. 决策：哪个 key 剩余最少？
...
async function getGlobalRateLimitStats(store) {
    ...
    try {
        ...
        let globalKeys = 0;
        let targetKeys = 0;
        let oldestExpireAt = null;
        for (const r of list) {
            if (r.scope === 'global') {
                globalKeys++;
            }
            else if (r.scope === 'target') {
                targetKeys++;
            }
            if (oldestExpireAt === null || r.expireAt < oldestExpireAt) {
                oldestExpireAt = r.expireAt;
            }
        }
        return {
