 ...
 */
async function consumeGlobalRateLimitWithFallback(input, config) {
    // Sprint 50: 按业务类型查找配置（db 热更新支持）
    const cfgResult = config
        ? { config, source: 'caller', enabled: true }
        : await (0, rate_limit_config_1.getRateLimitConfig)(input.type);
 ...
 * 全局版 peek（只查不消费）
 */
async function peekGlobalRateLimitWithFallback(input, config) {
    // Sprint 50: 按业务类型查找配置
    const cfgResult = config
        ? { config, source: 'caller', enabled: true }
        : await (0, rate_limit_config_1.getRateLimitConfig)(input.type);
...
async function withRateLimit(input, fn, config, store) {
    ...
    if (!effectiveConfig) {
        effectiveConfig = (0, rate_limit_config_1.getRateLimitConfigSync)(input.type).config;
    }
...
exports.default = {
    ...
    getStoreStats,
    // Sprint 50: 配置中心转发
    getRateLimitConfig: rate_limit_config_1.getRateLimitConfig,
    getRateLimitConfigSync: rate_limit_config_1.getRateLimitConfigSync,
};
