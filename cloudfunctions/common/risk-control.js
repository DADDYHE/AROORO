 ...
 */
exports.BOARDING_ACCEPT_CONFIG = {
    /** 短窗口高频接单阈值（5 分钟内） */
...
/** 检测 1：短窗口高频接单 */
function detectAcceptBurst(recentAccepts, now) {
    const cutoff = now - exports.BOARDING_ACCEPT_CONFIG.ACCEPT_BURST_WINDOW_MS;
    ...
    let level = 'low';
    if (count >= exports.BOARDING_ACCEPT_CONFIG.ACCEPT_BURST_THRESHOLD) {
        level = 'medium';
    }
    if (count >= exports.BOARDING_ACCEPT_CONFIG.ACCEPT_BURST_HIGH) {
        level = 'high';
    }
    return { hit: level !== 'low', count, level };
}
exports.detectAcceptBurst = detectAcceptBurst;
/** 检测 2：异常时段接单（凌晨） */
function detectAbnormalHour(now) {
    const d = new Date(now);
    const hour = d.getHours();
    const isAbnormal = hour >= exports.BOARDING_ACCEPT_CONFIG.ABNORMAL_HOUR_START
        && hour < exports.BOARDING_ACCEPT_CONFIG.ABNORMAL_HOUR_END;
    return {
        hit: isAbnormal,
...
exports.detectAbnormalHour = detectAbnormalHour;
/** 检测 3：大额订单接单 */
function detectLargeAcceptAmount(amountFen) {
    const amount = Math.max(0, Math.floor(amountFen || 0));
    if (amount >= exports.BOARDING_ACCEPT_CONFIG.HUGE_ACCEPT_FEN) {
        return { hit: true, level: 'high', amount };
    }
    if (amount >= exports.BOARDING_ACCEPT_CONFIG.LARGE_ACCEPT_FEN) {
        return { hit: true, level: 'medium', amount };
    }
    return { hit: false, level: 'low', amount };
}
exports.detectLargeAcceptAmount = detectLargeAcceptAmount;
/** 检测 4：新合作首接大额 */
function detectNewPartnerLargeAccept(partnerCreatedAt, amountFen, now) {
    if (!partnerCreatedAt) {
        return { hit: false, level: 'low', partnerAgeMs: 0 };
    }
    const partnerAgeMs = now - partnerCreatedAt;
    if (partnerAgeMs > exports.BOARDING_ACCEPT_CONFIG.NEW_PARTNER_WINDOW_MS) {
        return { hit: false, level: 'low', partnerAgeMs };
    }
    if (amountFen >= exports.BOARDING_ACCEPT_CONFIG.NEW_PARTNER_LARGE_FEN) {
        return { hit: true, level: 'medium', partnerAgeMs };
    }
    return { hit: false, level: 'low', partnerAgeMs };
}
exports.detectNewPartnerLargeAccept = detectNewPartnerLargeAccept;
/**
 * 主入口：寄养接单风控检测
 *
 ...
 * @throws 不抛错（best-effort）。失败时返回 level=low / action=allow
 */
async function detectBoardingAcceptRisk(ctx) {
    ...
    try {
        const recentAccepts = await safeList(db, 'orders', {
            hostId: partnerId,
            status: 'confirmed',
            updatedAt: { _op: 'gte', v: now - exports.BOARDING_ACCEPT_CONFIG.ACCEPT_BURST_WINDOW_MS },
        }, 50);
        const f2 = detectAcceptBurst(recentAccepts, now);
        if (f2.hit) {
            reasons.push(`ACCEPT_BURST:${f2.count}次/${exports.BOARDING_ACCEPT_CONFIG.ACCEPT_BURST_WINDOW_MS / 60000}分`);
    ...
    const allLevels = [f1.level];
    if (details.acceptBurst) {
        allLevels.push(details.acceptBurst.level);
    }
    if (details.abnormalHour) {
        allLevels.push(details.abnormalHour.level);
    }
    if (details.newPartnerLarge) {
        allLevels.push(details.newPartnerLarge.level);
    }
    const level = maxLevel(...allLevels);
    const action = levelToAction(level);
    return {
...
exports.detectBoardingAcceptRisk = detectBoardingAcceptRisk;
// =====================================================================
// 工具：db 拉取 + 快照转换
// =====================================================================
/**
