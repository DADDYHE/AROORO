...
exports.getHostEvaluations = exports.submitEvaluation = exports.handleBoardingOrder = exports.getBoardingOrderDetail = exports.getBoardingOrders = exports.checkDateAvailability = exports.calculatePrice = exports.adjustOrderPrice = exports.getOrderDetail = exports.cancelOrder = exports.getActivityOrderDetail = exports.getActivityOrders = exports.updateOrderStatus = exports.createOrder = exports.enrichOrders = exports.getOrders = void 0;
// Sprint 28 迁移说明：
...
const booking_key_1 = require("./common/booking-key");
 ...
 */
async function checkDateAvailabilityInternal(hostId, startDate, endDate, ownerId) {
    try {
        ...
        const existingOrders = await db.collection('orders')
            ...
            hostId,
            ownerId,
            status: db.command.in(booking_key_1.BOOKING_KEY_ACTIVE_STATUSES),
        })
...
function computeBoardingCharge(params) {
    ...
    const checkOutBefore = typeof hostData.checkOutBefore === 'string' && hostData.checkOutBefore
        ...
        : '12:00';
    return (0, boarding_pricing_1.computeBoardingAmount)({
        mode,
...
async function createOrder(event, _context, auth) {
    ...
    const { hostId, petIds, startDate, endDate, note, couponId, startAt, endAt } = event;
    ...
    const order = {
        ...
        bookingKey: (0, booking_key_1.buildBookingKey)({ hostId, startDate, endDate, startAt, endAt, ownerId }),
        createdAt: db.serverDate(),
        updatedAt: db.serverDate(),
        ...
        if (/E11000|duplicate key|bookingKey/i.test(rawMsg)) {
            ...
            try {
                const dupRes = await db.collection('orders')
                    .where({ bookingKey: order.bookingKey })
                    .field({ status: true })
                    .limit(1)
                    .get();
                const dupStatus = dupRes.data && dupRes.data[0]?.status;
...
async function updateOrderStatus(event, _context, auth) {
    ...
    // 活动 cancel 跳过顶部状态机校验（活动状态机 paid 仅允许 → completed/refunded，
    // 已支付取消需走下方退款分支，由 paymentService 处理 refunding/refunded）
    const odOrderType = od.orderType;
    if (!(status === 'cancelled' && odOrderType === 'activity')) {
    ...
    await db.collection('orders').doc(orderId).update({
        data: isTerminal
            ? { status, updatedAt: db.serverDate(), bookingKey: (0, booking_key_1.releasedBookingKey)(orderId) }
    ...
    if (order.outTradeNo) {
        ...
        try {
            ...
            logger.warn('cancelExpiredOrder: 关单调用失败，保守跳过本轮', {
                orderId: order._id, outTradeNo: order.outTradeNo, msg: e?.message,
            });
            return false;
        }
    }
 ...
 */
async function adjustOrderPrice(event, _context, auth) {
    const openid = auth?.openid;
    if (!openid) {
        throw err('AUTH_REQUIRED', '未登录');
    }
    const { orderId, newPrice, reason } = event;
    if (!orderId) {
        throw err('INVALID_PARAMS', '缺少订单ID');
    }
    ...
    // 改价留痕
    const adjustLog = Array.isArray(order.priceAdjustLog) ? [...order.priceAdjustLog] : [];
    adjustLog.push({
...
async function calculatePrice(event) {
    const { hostId, startDate, endDate, petIds, startAt, endAt } = event;
...
async function checkDateAvailability(event) {
    ...
    return (0, utils_1.handleSuccess)({ available: true }, '该档期可选');
}
...
async function handleBoardingOrder(event, _context, auth) {
    ...
    await db.collection('orders').doc(orderId).update({
        data: {
            status: newStatus,
            ...(booking_key_1.BOOKING_KEY_RELEASED_STATUSES.includes(newStatus)
                ? { bookingKey: (0, booking_key_1.releasedBookingKey)(orderId) }
                : {}),
