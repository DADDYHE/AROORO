...
/** 校验 YYYY-MM-DD 日期格式 */
function validateDateStr(value, fieldName) {
    ...
    if (isNaN(d.getTime())) {
        throw err('INVALID_PARAMS', `${fieldName}日期无效`);
    }
    return str;
}
/** 校验 HH:mm 时刻格式 */
function validateTimeStr(value, fieldName) {
    const str = String(value ?? '').trim();
...
/** 文本清洗（trim + 长度上限） */
function validateText(value, maxLength, fieldName) {
    const str = String(value ?? '').trim();
...
async function getHostProfileByOpenid(openid) {
...
async function checkPartnerHostingPermission(openid) {
...
// =====================================================================
async function createInvitation(event, _context, auth) {
    ...
    if (!hostProfile) {
        throw err('NOT_FOUND', '未找到您的寄养家庭档案，请先完成家庭入驻');
    }
    const hostId = hostProfile._id;
    const { startDate, endDate, startAt, endAt, petCount, totalPrice, note } = event;
    ...
    const invitation = {
        _id: (0, utils_1.generateId)('binv', openid),
        ...
        hostSnapshot: {
            ...
            addressPublic: hostProfile.addressPublic || '',
        },
        startDate: safeStartDate,
        endDate: safeEndDate,
        startAt: safeStartAt,
    ...
    return (0, utils_1.handleSuccess)({ invitation }, '开单成功');
}
// =====================================================================
// Handler 2: getMyInvitations - 家庭的开单列表
// =====================================================================
async function getMyInvitations(event, _context, auth) {
    const openid = auth?.openid;
    if (!openid) {
        throw err('AUTH_REQUIRED', '未登录');
    }
    ...
    const hostProfile = await getHostProfileByOpenid(openid);
    if (!hostProfile) {
        throw err('NOT_FOUND', '未找到您的寄养家庭档案');
    }
    ...
    if (orderStatusFilter) {
        const filledRes = await db.collection(COLLECTION)
            ...
            .limit(200)
            .get();
        const filledList = (filledRes.data || []);
        const orderIds = filledList.map(x => x.orderId).filter(Boolean);
        const orderMap = {};
        if (orderIds.length) {
    ...
    const orderIds = list.map(x => x.orderId).filter(Boolean);
    if (orderIds.length) {
        const ordRes = await db.collection('orders')
            .where({ _id: db.command.in(orderIds) })
            .field({ _id: true, status: true })
        ...
        const orderMap = {};
        (ordRes.data || []).forEach(o => { orderMap[o._id] = o.status || ''; });
        list = list.map(x => x.orderId ? ({ ...x, orderStatus: orderMap[x.orderId] }) : x);
    }
    return (0, utils_1.handleSuccess)({
        list,
...
// =====================================================================
async function updateInvitation(event, _context, auth) {
    const openid = auth?.openid;
    if (!openid) {
        throw err('AUTH_REQUIRED', '未登录');
    }
    await checkPartnerHostingPermission(openid);
    const { invitationId, startDate, endDate, startAt, endAt, petCount, totalPrice, note } = event;
    if (!invitationId) {
        throw err('INVALID_PARAMS', '缺少邀请 ID');
    }
    const invRes = await db.collection(COLLECTION).doc(invitationId).get();
    if (!invRes.data) {
        throw err('NOT_FOUND', '邀请不存在');
    }
    ...
    return (0, utils_1.handleSuccess)(null, '保存成功');
}
// =====================================================================
// Handler 3: cancelInvitation - 家庭取消邀请（仅 active 可取消）
// =====================================================================
async function cancelInvitation(event, _context, auth) {
    const openid = auth?.openid;
    if (!openid) {
        throw err('AUTH_REQUIRED', '未登录');
    }
    ...
    return (0, utils_1.handleSuccess)(null, '已取消');
}
// =====================================================================
// Handler 4: getInvitationByCode - 公开读（用户从分享进入）
// =====================================================================
async function getInvitationByCode(event) {
    const { code } = event;
    const safeCode = String(code ?? '').trim();
    if (!safeCode || safeCode.length > 32) {
        throw err('INVALID_PARAMS', '邀请码无效');
    }
...
// =====================================================================
async function submitInvitation(event, _context, auth) {
    const openid = auth?.openid;
    if (!openid) {
        throw err('AUTH_REQUIRED', '未登录');
    }
    const { code, petIds, note } = event;
    const safeCode = String(code ?? '').trim();
    if (!safeCode) {
        throw err('INVALID_PARAMS', '缺少邀请码');
    }
    ...
    if (inv.status !== 'active') {
        if (inv.status === 'filled') {
            throw err('STATE_INVALID', '该邀请已被填写');
        }
    ...
    if (!claimRes || claimRes.stats?.updated === 0) {
        // 并发：他人刚占用
        const retryRes = await db.collection(COLLECTION).doc(inv._id).get();
        const finalStatus = retryRes.data?.status;
        if (finalStatus === 'filled') {
            throw err('STATE_INVALID', '手慢了，该邀请刚被他人填写');
        }
    ...
    const calendarDays = Math.max(1, Math.round((Date.parse(`${endDate}T00:00:00Z`) - Date.parse(`${startDate}T00:00:00Z`)) / 86400000));
    let ownerInfo = {};
    try {
        ...
        logger.warn('submitInvitation.users.fetch', { code: e.errCode, msg: e.message });
    }
    // hostInfo 完整快照（与 createOrder 对齐：剔除敏感字段后存订单）
    const hostInfo = { ...inv.hostSnapshot };
    try {
        const hostDoc = await db.collection('hostProfiles').doc(inv.hostId).get();
        if (hostDoc.data) {
            const fullHost = { ...hostDoc.data };
            SENSITIVE_HOST_FIELDS.forEach(f => { delete fullHost[f]; });
            Object.assign(hostInfo, fullHost);
        }
    }
    catch (e) {
        logger.warn('submitInvitation.hostProfiles.fetch', { msg: e.message });
    }
    ...
    const order = {
        ...
        hostId: inv.hostId,
        type: 'boarding',
        organizerId: hostInfo.openid || inv.hostOpenid,
        petIds,
        startDate,
        endDate,
        startAt: inv.startAt,
        ...
        hostName: (inv.hostSnapshot.hostName) || '',
        autoConfirmed: false, // 支付成功后置 true（paymentService 回调）
    };
    const orderId = (0, utils_1.generateId)('order', openid);
    order._id = orderId;
    try {
        ...
        logger.error('submitInvitation.order.create_failed', { invitationId: inv._id, msg: e?.message });
        try {
            ...
            logger.error('submitInvitation.revert_failed', { invitationId: inv._id, msg: revertErr?.message });
        }
        throw err('ORDER_CREATE_FAILED', '订单创建失败，请重试');
    }
    // 回填 orderId（best-effort；失败不影响主流程，家庭列表可看到 filled 状态）
    try {
        await db.collection(COLLECTION).doc(inv._id)
            .update({ data: { orderId, updatedAt: db.serverDate() } });
    }
    catch (e) {
        logger.warn('submitInvitation.orderId_backfill_failed', { invitationId: inv._id, msg: e?.message });
    }
    logger.info('submitInvitation', { invitationId: inv._id, orderId });
    return (0, utils_1.handleSuccess)({ orderId, invitationId: inv._id }, '提交成功');
}
// =====================================================================
// Handler 6: getInviteQrCode - 家庭获取小程序码（云端缓存）
// =====================================================================
async function getInviteQrCode(event, _context, auth) {
    const openid = auth?.openid;
    if (!openid) {
        throw err('AUTH_REQUIRED', '未登录');
    }
    ...
    // 已生成过 → 直接返回缓存的 fileID
    if (inv.qrCodeFileId) {
        return (0, utils_1.handleSuccess)({ fileId: inv.qrCodeFileId }, '获取成功');
    }
    ...
    return (0, utils_1.handleSuccess)({ fileId: uploadRes.fileID }, '获取成功');
}
// =====================================================================
