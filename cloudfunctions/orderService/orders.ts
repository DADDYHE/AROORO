...
export async function getOrders(event: EventLike, _context: ContextLike, auth: AuthLike | null): HandlerResult {
  ...
  for (const doc of (result.data || []) as Array<Record<string, unknown>>) {
    const d = doc as unknown as { _id?: string, status?: string, timeoutAt?: number, couponId?: string }
...
export async function updateOrderStatus(event: EventLike, _context: ContextLike, auth: AuthLike | null): HandlerResult {
  ...
  if (status === 'cancelled' && od.paymentStatus !== 'paid') {
    try {
      await unlockOrderCoupons(orderId, (od as { couponId?: string }).couponId)
    } catch (couponErr) {
      logger.warn('updateOrderStatus.unlockCoupons', { orderId, msg: (couponErr as Error)?.message })
    }
  }
  ...
  try {
    ...
    if (expiredIds.length > 0) {
      await db.collection('user_coupons')
        .where({ _id: (db.command as { in: (arr: string[]) => unknown }).in(expiredIds), status: 'locked' })
        .update({ data: { status: 'expired', updatedAt: db.serverDate() } })
    }
    if (unusedIds.length > 0) {
      await db.collection('user_coupons')
        .where({ _id: (db.command as { in: (arr: string[]) => unknown }).in(unusedIds), status: 'locked' })
        .update({ data: { status: 'unused', updatedAt: db.serverDate() } })
    }
  } catch (e) {
...
async function cancelExpiredOrderIfNeeded(order: { _id?: string, status?: string, timeoutAt?: number, couponId?: string }): Promise<boolean> {
  ...
  if (!cancelRes.updated || cancelRes.updated === 0) {return false}
  if (order.couponId) {
...
export async function getOrderDetail(event: EventLike, _context: ContextLike, auth: AuthLike | null): HandlerResult {
  ...
  const odPay = order.data as { status?: string, timeoutAt?: number, couponId?: string, _id?: string }
  if (await cancelExpiredOrderIfNeeded(odPay as never)) {
    ;(order.data as { status?: string }).status = 'cancelled'
  }
