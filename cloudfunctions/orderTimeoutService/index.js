...
function pushError(result, err) {
    ...
    if (isResourceRollbackFailure) {
        void recordAlert('critical', 'orderTimeout.resource_rollback_failed', '超时取消的资源回退失败，可能造成库存/名额/团累计错账，需人工对账', { ...err }).catch(() => { });
    }
...
async function completeActivityOrders(result, now) {
    try {
        ...
        for (const reg of paidRegs) {
            ...
            if (!endedActivityIds.has(reg.activityId)) {
                continue;
            }
            try {
...
async function main(event, _context) {
    ...
    if (event.action === 'closePrepay') {
        ...
        if (!outTradeNo) {
            return handleSuccess({ close: false, tradeState: 'UNKNOWN' }, '缺少 outTradeNo');
        }
