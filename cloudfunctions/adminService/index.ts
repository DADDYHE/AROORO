...
export const main = async (event: CloudEvent, context: CloudContext): Promise<unknown> => {
  ...
  if (event.accessToken) {
    try {
      ...
      const auth: AuthLike = { openid: decoded.openid, partnerId: decoded.adminId, isPartner: jwtEnrichment?.isPartner || decoded.isPartner || false, isSuperAdmin: jwtEnrichment ? jwtEnrichment.roles.includes('super_admin') : (decoded.isSuperAdmin || false), _isHttpAuth: true }
      if (jwtEnrichment) {
