...
const listI18nOverrides = withErrorHandling(async (event = {}) => {
  ...
  if (prefix && typeof prefix === 'string') {
    ...
    filter.key = db.RegExp({ regexp: `^${escapeRegExp(prefix)}`, options: 'i' })
  }
 ...
 * 返回：{ baseLocale, missingByLocale: { [locale]: [{ key, availableIn: [locale] }] }, totalMissing, totalKeys }
 */
const findMissingTranslations = withErrorHandling(async (event = {}) => {
  ...
  return handleSuccess({
    ...
    totalMissing,
    missingByLocale,
  }, `扫描 ${totalKeys} 个 key，发现 ${totalMissing} 处缺失`)
})
 ...
 *
 * 入参：无
 * 返回：{ totalDocs, activeDocs, disabledDocs, byKey, byLocale, byStatus, lastUpdatedAt }
 */
const getI18nOverrideStats = withErrorHandling(async () => {
  const allRes = await db.collection(COLLECTION).where({}).limit(2000).get()
  ...
  for (const d of docs) {
    if (!d) { continue }
    if (d.key) { byKey[d.key] = (byKey[d.key] || 0) + 1 }
    if (d.locale) { byLocale[d.locale] = (byLocale[d.locale] || 0) + 1 }
    if (d.status === 'active') { byStatus.active++ }
    else if (d.status === 'disabled') { byStatus.disabled++ }
    else { byStatus.other++ }
    if (d.updatedAt && (!lastUpdatedAt || d.updatedAt > lastUpdatedAt)) {
