...
async function updateCouponTemplate(event, context, auth) {
  ...
  // active/inactive 状态下只允许更新领取相关设置
  const activeAllowedFields = ['claimable', 'perUserLimit', 'popupEnabled', 'popupPage']
  const systemFields = ['action', 'data', 'adminOpenid', 'timestamp', 'token', 'HTTP_CONTEXT']
  if (template.status !== 'draft') {
    const hasOtherFields = Object.keys(event).some(
      k => k !== 'templateId' && !activeAllowedFields.includes(k) && !systemFields.includes(k)
    )
    if (hasOtherFields) {
      // 忽略不允许修改的字段，仅更新允许的字段
      const updateData = { updatedAt: db.serverDate() }
      activeAllowedFields.forEach(k => {
        if (event[k] !== undefined) {updateData[k] = event[k]}
      })
      await db.collection('coupon_templates').doc(templateId).update({ data: updateData })
      return handleSuccess(null, '已更新领取设置')
    }
  }
  ...
  if (event.stock !== undefined) {
    const newStock = Number(event.stock) || 0
    const distributedCount = template.stock - template.remaining
    const newRemaining = Math.max(0, newStock - distributedCount)
  ...
  for (const targetOpenid of userIds) {
    ...
    try {
      const existingCount = await db.collection('user_coupons')
        .where({ templateId, ownerId: targetOpenid, status: _.in(['unused', 'locked']) })
        .count()
...
async function getGrantList(event, context, auth) {
  ...
  if (keyword) { where.templateName = db.RegExp({ regexp: keyword, options: 'i' }) }
