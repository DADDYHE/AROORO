...
class AuthService {
  ...
  _wxLogin() {
    return new Promise((resolve, reject) => {
      wx.login({
        success: res => {
          if (res.code) {
            logger.info('wx.login 成功')
            resolve(res.code)
          } else {
            logger.error('wx.login 失败', { error: res.errMsg })
            reject(new Error(`wx.login 失败: ${res.errMsg}`))
          }
        },
        fail: err => {
          logger.error('wx.login 调用失败', { error: err.message || String(err) })
  ...
  _wxCheckSession() {
    return new Promise(resolve => {
      wx.checkSession({
        success: () => {
          logger.info('wx.checkSession: 微信会话有效')
          resolve(true)
        },
        fail: () => {
          logger.info('wx.checkSession: 微信会话已过期')
          resolve(false)
        },
      })
    })
  ...
  async _ensureWxSession() {
    ...
    logger.info('微信会话已过期，重新 wx.login()')
    try {
      await this._wxLogin()
      return true
    } catch (error) {
      logger.warn('wx.login 刷新会话失败', { error: error.message })
      return false
    }
  }
  ...
  async _doLogin(app, options = {}) {
    try {
      try {
        ...
        logger.warn('wx.login 失败，仍尝试调用云函数', { error: wxLoginError.message })
      }
      ...
      try {
        ...
        logger.warn('清除退出标记失败', { error: e.message || String(e) })
      }
      ...
      logger.error('授权登录失败', { error: error.message || String(error) })
  ...
  async _doCloudLogin(app, options = {}) {
    try {
      ...
      logger.error('云函数登录失败', { error: error.message || String(error) })
      throw error
    }
  }
  ...
  async _refreshAdminStatus(app) {
    try {
      ...
      logger.warn('刷新合作伙伴状态失败', { error: e.message })
    }
  }
  ...
  _persistLoginState(user) {
    try {
      ...
      logger.warn('持久化登录状态失败', { error: e.message || String(e) })
    }
  }
  ...
  _syncUserInfoToGlobal(updatedInfo) {
    ...
    try {
      ...
      logger.warn('同步缓存失败', { error: e.message || String(e) })
    }
  }
  ...
  isLoggedIn() {
    const app = getApp()
    const result = Boolean(app.globalData.isLoggedIn && app.globalData.userInfo)
    logger.info('isLoggedIn', {
      result,
      hasIsLoggedIn: Boolean(app.globalData.isLoggedIn),
  ...
  async startLogin() {
    ...
    logger.info('startLogin', { isLoggedIn: loggedIn })
  ...
  async logout() {
    ...
    try {
      ...
      logger.warn('设置退出标记失败', { error: e.message || String(e) })
    }
