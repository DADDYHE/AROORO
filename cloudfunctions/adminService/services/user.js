...
async function getEnhancedDashboardStats(event, context, auth) {
  try {
    ...
    ;(mallTypeAgg.data || []).forEach(t => {
      ...
      ordersByType[typeName] = { name: typeName, count: t.count }
    })
    if (tuanTypeAgg.data && tuanTypeAgg.data[0] && tuanTypeAgg.data[0].count > 0) {
      ordersByType.tuan = { name: 'tuan', count: tuanTypeAgg.data[0].count }
    }
    ...
    const [mallRevAgg, tuanRevAgg, feedingRevAgg, activityRevAgg] = await Promise.all([
      db.collection('orders').aggregate()
        .match({ createdAt: _.gte(thirtyDaysAgo) })
        .group({ _id: '$type', revenue: { $sum: { $toDouble: { $ifNull: ['$totalPrice', { $ifNull: ['$price', 0] }] } } } })
        .end(),
      db.collection('tuan_orders').aggregate()
        .match({ createdAt: _.gte(thirtyDaysAgo), status: _.neq('cancelled') })
    ...
    ;(mallRevAgg.data || []).forEach(t => {
      const typeName = t._id || 'mall'
      revenueByType[typeName] = { name: typeName, amount: Number((t.revenue || 0).toFixed(2)) }
    })
    if (tuanRevAgg.data && tuanRevAgg.data[0] && tuanRevAgg.data[0].revenue > 0) {
      revenueByType.tuan = { name: 'tuan', amount: Number(tuanRevAgg.data[0].revenue.toFixed(2)) }
    }
...
async function getFinanceOverview(event, context, auth) {
  try {
    ...
    const ORDER_TYPE_MAP = {
      mall: { collection: 'orders', orderType: 'mall' },
      boarding: { collection: 'orders', orderType: 'hosting' },
      activity: { collection: 'orders', orderType: 'activity' },
      tuan: { collection: 'tuan_orders', orderType: 'tuan' },
      feeding: { collection: 'feedingOrders', orderType: 'feeding' },
    }
    ...
    for (const [type, config] of Object.entries(ORDER_TYPE_MAP)) {
      const matchCond = { status: 'confirmed', orderType: config.orderType }

      const [allRes, recentRes] = await Promise.all([
        db.collection(config.collection).where(matchCond).field({ totalPrice: true, completedAt: true }).limit(1000).get(),
        db.collection(config.collection).where(matchCond).orderBy('completedAt', 'desc').limit(5).field({ _id: true, totalPrice: true, completedAt: true }).get(),
      ])
      ...
      ;(allRes.data || []).forEach(order => {
        ...
        typeTotal += amount
        if (order.completedAt && new Date(order.completedAt) >= monthStart) {
          typeMonthly += amount
        }
      })
