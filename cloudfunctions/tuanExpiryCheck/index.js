 ...
 */
async function fetchPendingOrderIds(dealIds) {
    const orderIds = [];
...
async function cleanupDownstreamForDeals(dealIds) {
    ...
    result.cancelledOrders = await batchUpdateByIds(DOWNSTREAM.ORDERS, orderIds, { status: 'pending_payment' }, {
        status: 'cancelled',
        cancelReason: CANCEL_REASON,
        cancelledAt: db.serverDate(),
        updatedAt: db.serverDate(),
    ...
    for (let i = 0; i < orderIds.length; i += exports.BATCH_LIMIT) {
        const batch = orderIds.slice(i, i + exports.BATCH_LIMIT);
    ...
    result.cancelledCommissions = await batchUpdateByIds(DOWNSTREAM.TUAN_COMMISSIONS, orderIds, { status: 'pending' }, {
        status: 'cancelled',
        cancelledAt: db.serverDate(),
        updatedAt: db.serverDate(),
    });
