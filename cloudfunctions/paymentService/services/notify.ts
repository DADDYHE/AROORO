...
async function applyPaidStatus(
  ...
  if (orderType === 'order') {
    ...
    const o = existingOrder as unknown as Record<string, unknown>
  ...
  try {
    ...
    const txCol = transaction.collection(collection) as unknown as {
      where: (cond: Record<string, unknown>) => {
        update: (u: { data: Record<string, unknown> }) => Promise<{ updated?: number }>
      }
    }
 ...
 */
async function triggerOrderCouponUse(order: NotifyOrderDoc, business: string): Promise<void> {
  ...
  try {
    ...
    if (!useRes || useRes.code !== 0) {
      ...
      try {
        await recordAlert('warning', `paymentNotify.${business}.useCoupon.failed`,
          `${business}订单优惠券核销失败，需人工核对`,
          {
    ...
    try {
      await recordAlert('warning', `paymentNotify.${business}.useCoupon.exception`,
        `${business}订单优惠券核销异常，需人工核对`,
        {
          orderId, outTradeNo: order.outTradeNo, couponId,
...
export async function paymentNotify(
  ...
  try {
    ...
    //   仅用于在"缓存多把公钥做轮换"时选择对应公钥验签；官方验签流程【不要求】比对 serial。
    //   本项目使用固定微信支付公钥（WECHAT_PAY_CERTIFICATE），直接用其对
    //   时间戳\n随机串\n报文\n 做 SHA256-RSA 验签即为完整安全校验（官方标准）。
    //   serial 缺失仍拒绝（微信回调必带），但【不做 serial 比对】——
    //   原实现（0162dc8）拿商户 API 证书序列号比对 serial 导致 7/25 后全部回调 401 被拒（P0 资损）。
    //   曾引入可选 platformSerialNo 配置做严格比对，后移除：无公钥轮换场景下比对零增益，
    //   且配置填错（公钥ID/证书序列号混淆）会再次全量拒回调，属高风险配置项。
    if (!serial) {
      ...
      if (list.length > 0) {
        ...
        if (existingOrder.status === 'cancelled') {
          ...
          try {
            await recordAlert(
              'warning',
              'paymentNotify.skipped_cancelled_order',
              '支付回调命中已取消订单（可能关单失败导致用户仍可付款），需人工对账退款',
        ...
        const amountObj = (orderInfo.amount || {}) as { total?: number }
        ...
        if (paidSuccess) {
          ...
          if (orderType === 'activity' && existingOrder.activityId) {
            Promise.resolve()
              .then(async () => {
                const actRes = await db.collection('activities').doc(existingOrder.activityId as string).get()
                const creatorId = actRes.data && (actRes.data as { createdBy?: string }).createdBy
                if (creatorId) {
                  const { createActivityIncomeRecords } = require('../common/service-income-utils')
                  await createActivityIncomeRecords(existingOrder.activityId as string, creatorId)
                }
              })
              .catch((incomeErr) => {
                logger.error('paymentNotify.activityIncome.failed', {
                  orderId: existingOrder._id,
                  activityId: existingOrder.activityId,
                  msg: incomeErr && (incomeErr as Error).message,
                })
              })
          }
