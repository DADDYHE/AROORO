...
exports.createPayment = (0, errors_1.withErrorHandling)(async (event, context, auth) => {
    ...
    if (orderData.status === 'cancelled') {
        throw (0, errors_1.err)('ORDER_STATUS_CHANGED', '订单已取消，无法支付', { orderId, status: orderData.status });
    }
    // 2026-09-06：待支付单超时校验——前端倒计时归零后、cron 取消前的窗口期内禁止支付，
    //   消除「用户付款与超时取消竞态」（扣款但订单已取消）。旧单无 timeoutAt 字段不校验（兼容）
    const odTimeout = orderData.timeoutAt;
    if (orderType === 'order' && orderData.status === 'pending_payment' && odTimeout !== undefined && odTimeout !== null) {
        ...
        if (Number.isFinite(timeoutAt) && timeoutAt > 0 && timeoutAt < Date.now()) {
            ...
            throw (0, errors_1.err)('ORDER_STATUS_CHANGED', '订单已超时，请重新下单', { orderId, timeoutAt });
        }
    }
...
exports.confirmPayment = (0, errors_1.withErrorHandling)(async (event, _context, _auth) => {
    ...
    if (paidAmountFen !== null && Number.isFinite(paidAmountFen)) {
        ...
        if (expectedFen > 0 && expectedFen !== Math.round(paidAmountFen)) {
            ...
            await recordAlert('critical', 'confirmPayment.amount_mismatch', '确认支付时微信实付金额与订单金额不一致，需人工对账', { outTradeNo, orderType, orderId: existingOrder._id, paidFen: paidAmountFen, expectedFen });
