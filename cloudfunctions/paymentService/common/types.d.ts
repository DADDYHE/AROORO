...
export type BusinessErrorCode = 'INVALID_PARAMS' | 'MISSING_REQUIRED' | 'AUTH_REQUIRED' | 'TOKEN_EXPIRED' | 'TOKEN_INVALID' | 'WX_LOGIN_FAILED' | 'PERMISSION_DENIED' | 'PARTNER_REQUIRED' | 'ADMIN_REQUIRED' | 'SUPER_ADMIN_REQUIRED' | 'NOT_FOUND' | 'ORDER_NOT_FOUND' | 'USER_NOT_FOUND' | 'HOST_NOT_FOUND' | 'PET_NOT_FOUND' | 'PRODUCT_NOT_FOUND' | 'COUPON_NOT_FOUND' | 'ACTIVITY_NOT_FOUND' | 'BANNER_NOT_FOUND' | 'DUPLICATE_KEY' | 'DB_ERROR' | 'DATA_ERROR' | 'ORDER_CREATE_FAILED' | 'ORDER_STATUS_INVALID' | 'ORDER_STATUS_CHANGED' | 'ORDER_ALREADY_PAID' | 'ORDER_ALREADY_REFUNDED' | 'ORDER_TIMEOUT' | 'REFUND_FAILED' | 'PAYMENT_CREATE_FAILED' | 'PAYMENT_NOTIFY_INVALID' | 'PAYMENT_AMOUNT_MISMATCH' | 'WECHAT_API_ERROR' | 'ENCRYPT_FAILED' | 'DECRYPT_FAILED' | 'INVALID_PAYLOAD' | 'INTERNAL_ERROR' | 'SERVICE_UNAVAILABLE' | 'RATE_LIMITED' | 'IDEMPOTENT_REPLAY' | 'UNKNOWN_ACTION' | 'STATE_INVALID' | 'CATEGORY_HAS_PRODUCTS' | 'COUPON_LIMIT_REACHED' | 'COUPON_STATUS_INVALID' | 'COUPON_LOCK_FAILED' | 'COUPON_USE_FAILED' | 'STOCK_INSUFFICIENT' | 'ACTIVITY_HAS_REGISTRATIONS' | 'BUSINESS_ERROR' | 'RISK_REJECT' | 'RISK_PENDING' | 'RISK_PASS';
/** 错误规格（注册表条目） */
