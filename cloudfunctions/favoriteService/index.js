 ...
 */
function buildFavoriteId(openid, targetType, targetId) {
 ...
 * - 收藏为低频操作，多一次点查可接受
 */
async function assertTargetExists(dbInstance, targetType, targetId) {
...
async function removeFavorite(event, openid, dbInstance) {
    ...
    const res = await dbInstance.collection(exports.COLLECTION)
...
async function getFavorites(event, openid, dbInstance) {
    ...
    const safePageSize = Math.min(Math.max(1, Math.floor(Number(pageSize) || exports.DEFAULT_PAGE_SIZE)), exports.MAX_PAGE_SIZE);
    ...
    const [countRes, listRes] = await Promise.all([
        dbInstance.collection(exports.COLLECTION).where(where).count(),
        dbInstance.collection(exports.COLLECTION)
            .where(where)
