...
const cloud = require('wx-server-sdk') as {
  ...
  database: () => {
    collection: (name: string) => {
      // M1: 扩展类型定义，支持 where().limit().get() 和 doc(id).update()
      where: (q: Record<string, unknown>) => {
        update: (params: { data: Record<string, unknown> }) => Promise<{ stats: { updated: number } }>
        limit: (n: number) => {
          get: () => Promise<{ data: UserCouponDoc[] }>
        }
        get: () => Promise<{ data: UserCouponDoc[] }>
      }
      doc: (id: string) => {
        update: (params: { data: Record<string, unknown> }) => Promise<{ stats: { updated: number } }>
        get: () => Promise<{ data: UserCouponDoc | null }>
      }
    }
    command: {
      lt: (d: Date) => unknown
...
export const STUCK_LOCKED_STATUS: CouponStatus = 'locked'
export const STUCK_LOCKED_DAYS = 7
export const STUCK_LOCKED_LIMIT = 100  // 单次最多处理 100 张卡死券，避免影响 cron 性能
...
export async function main(event: CloudEvent, _context: CloudContext): Promise<unknown> {
  ...
  try {
    ...
    const stuckThreshold = new Date(now.getTime() - STUCK_LOCKED_DAYS * 24 * 60 * 60 * 1000)
    const stuckQueryRes = await db.collection(COLLECTION)
      .where({
        status: STUCK_LOCKED_STATUS,
        endTime: _.lt(stuckThreshold),
      })
      .limit(STUCK_LOCKED_LIMIT)
      .get()

    const stuckCoupons = stuckQueryRes.data || []
    for (const coupon of stuckCoupons) {
      if (!coupon._id) { continue }
      try {
        await db.collection(COLLECTION).doc(coupon._id).update({
          data: {
            status: NEW_STATUS,
    ...
    if (rounds === MAX_ROUNDS && totalUpdated === MAX_ROUNDS * BATCH_LIMIT) {
      ...
      // M3: 达到上限也是异常情况，主动告警
      try {
        await recordAlert('warn', 'coupon.expiry.max.rounds',
          '优惠券过期检查达到最大轮次，可能存在未处理的过期券',
          { totalUpdated, maxRounds: MAX_ROUNDS, batchLimit: BATCH_LIMIT })
      } catch (_) { /* best-effort */ }
    }
    ...
    if (stuckLockedUpdated > 0) {
      logger.warn('stuck locked coupons expired', {
        ...
        threshold: `${STUCK_LOCKED_DAYS} days before ${now.toISOString()}`,
      })
      try {
        await recordAlert('warn', 'coupon.expiry.stuck.locked',
          `检测到 ${stuckLockedUpdated} 张卡死券已被标记为过期（locked 状态超过 ${STUCK_LOCKED_DAYS} 天）`,
          {
            count: stuckLockedUpdated,
    ...
    try {
      await recordAlert('critical', 'coupon.expiry.check.failed',
        '优惠券过期检查失败',
        {
