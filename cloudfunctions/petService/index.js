...
exports.main = exports.handlers = exports.getPet = exports.getPetDetail = exports.getPetList = exports.deletePet = exports.updatePet = exports.createPet = exports.sanitizeHealthInfo = exports.convertWeight = void 0;
// =====================================================================
...
const PET_FIELDS = (() => {
    const result = { ...PET_DETAIL_FIELDS };
...
// H6: 辅助函数：文本字段校验
// =====================================================================
function validateTextField(value, maxLength, fieldName) {
 ...
 */
function sanitizeHealthInfo(input) {
...
exports.sanitizeHealthInfo = sanitizeHealthInfo;
/**
...
// Handler 1: createPet
// =====================================================================
exports.createPet = withErrorHandling(async (event, _context, auth) => {
    const { openid } = auth;
    if (!openid) {
    ...
    // H4: 限流 + 单用户宠物数量上限校验
    return withRateLimit({ userId: openid, type: 'createPet' }, async () => {
        ...
        const result = { id: petData._id || '', pet: sanitizePet(petData) };
        ...
        }).catch((e) => logger.warn('writeOperationLog failed', { msg: e.message }));
...
// Handler 2: updatePet
// =====================================================================
exports.updatePet = withErrorHandling(async (event, _context, auth) => {
    const { openid } = auth;
    if (!openid) {
        throw err('AUTH_REQUIRED', '未登录');
    }
    ...
    if (updateData.type !== undefined && !VALID_TYPES.includes(updateData.type)) {
        throw err('INVALID_PARAMS', '宠物类型无效');
    }
    if (updateData.gender !== undefined && !VALID_GENDERS.includes(updateData.gender)) {
        throw err('INVALID_PARAMS', '性别无效');
    }
    ...
    const filteredFields = filterFields(FIELD_WHITELISTS.pet, updateData);
    const updateFields = {
        updatedAt: db.serverDate(),
        ...filteredFields,
    };
    ...
    if (updateFields.healthInfo !== undefined) {
        ...
        updateFields.healthInfo = { ...sanitized, healthUpdatedAt: db.serverDate() };
    }
    ...
    // P3 修复：返回脱敏后的宠物数据（不含 ownerId/_openid）
    const result = { pet: sanitizePet(updatedPet) };
    // M9: 记录操作日志
    writeOperationLog({
        module: 'pet',
        action: 'update',
...
// =====================================================================
exports.deletePet = withErrorHandling(async (event, _context, auth) => {
    const { openid } = auth;
    if (!openid) {
        throw err('AUTH_REQUIRED', '未登录');
    }
    ...
    const existingPet = existingResult.data;
    if (existingPet.ownerId !== openid) {
    ...
    if (avatarUrl && avatarUrl.startsWith('cloud://')) {
        cloud.deleteFile({ fileList: [avatarUrl] }).catch((e) => {
...
// =====================================================================
exports.getPetList = withErrorHandling(async (event, _context, auth) => {
    const { openid } = auth;
    if (!openid) {
        throw err('AUTH_REQUIRED', '未登录');
    }
...
// =====================================================================
exports.getPetDetail = withErrorHandling(async (event, _context, auth) => {
    const { petId } = event;
    if (!petId) {
        throw err('INVALID_PARAMS', '宠物 ID 不能为空');
    }
    ...
    if (cachedPet) {
        ...
        return handleSuccess({ pet: publicPet }, '获取成功');
    }
    ...
    const result = await db.collection('pets')
        ...
        .field({ ...PET_DETAIL_FIELDS, ownerId: true })
...
// =====================================================================
exports.getPet = withErrorHandling(async (event, context, auth) => {
    // H2: 委托 getPetDetail，统一走缓存路径 + 字段投影
    return (0, exports.getPetDetail)(event, context, auth);
});
// =====================================================================
// Handlers 聚合
// =====================================================================
exports.handlers = {
    createPet: exports.createPet,
    updatePet: exports.updatePet,
    deletePet: exports.deletePet,
    getPet: exports.getPet,
    getPetList: exports.getPetList,
    getPetDetail: exports.getPetDetail,
};
// =====================================================================
...
try {
    const { db: bootstrapDb } = initCloud();
    bootstrapRateLimit(bootstrapDb, {
    ...
    logger.error('bootstrapRateLimit failed', { msg: e?.message });
    if (e instanceof Error && e.code === 'RATE_LIMIT_BOOTSTRAP_FAILED') {
        recordAlert('warning', 'petService.rate_limit.bootstrap.failed', `petService 限流 bootstrap 失败：${e.message}`, { service: 'petService', stack: e.stack }).catch((alertErr) => {
...
async function main(event, context) {
    ...
    try {
        ...
        if (isBusinessError(error)) {
            logger.warn(action, { code: error.code, msg: error.message });
            return toResponse(error);
        }
        // 未知错误走 handleError，并触发持久化告警
        logger.error(action, error);
        recordAlert('critical', `petService.${action}.failed`, `petService ${action} 执行失败：${error?.message}`, { action, stack: error?.stack }).catch(() => { });
        return handleError(error, error?.message || '操作失败', ERROR_CODES.SERVER);
    }
}
exports.main = main;
// =====================================================================
