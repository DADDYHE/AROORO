...
async function getReferralStats(event, context, auth) {
  ...
  try {
    ...
    if (!user) {return handleSuccess({ totalInvited: 0, consumingCount: 0, totalSpent: '0.00' })}
    ...
    if (invitedOpenids.length > 0) {
      ...
      try {
        ...
        ;(feedRes.data || []).forEach(o => { if (o.ownerId) {spenderOpenids.add(o.ownerId)} totalSpent += (Number(o.totalPrice) || Number(o.price) || 0) })
      } catch (e) {
      ...
      try {
        ...
        ;(tuanRes.data || []).forEach(o => { if (o.ownerId) {spenderOpenids.add(o.ownerId)} totalSpent += (Number(o.totalPrice) || Number(o.price) || 0) })
      } catch (e) {
      ...
      try {
        ...
        ;(actRes.data || []).forEach(o => { if (o.ownerId) {spenderOpenids.add(o.ownerId)} totalSpent += (Number(o.totalPrice) || Number(o.price) || 0) })
      } catch (e) {
...
async function getMyInvitedUsers(event, context, auth) {
  ...
  try {
    ...
    try {
      ...
      logger.warn('getMyInvitedUsers.users.fetch', { openid, code: e.errCode, msg: e.message })
    }
...
async function getReferralOrderStats(event, context, auth) {
  ...
  try {
    ...
    try {
      ...
      logger.warn('getReferralOrderStats.users.fetch', { openid, code: e.errCode, msg: e.message })
    }
    if (!user) {return handleSuccess({ totalOrders: 0, totalCommission: 0, pendingCommission: 0, settledCommission: 0 })}
    ...
    let totalOrders = 0, totalCommission = 0, pendingCommission = 0, settledCommission = 0
    ;(res.data || []).forEach(c => {
      totalOrders++
...
module.exports = { getReferralStats, getMyInvitedUsers, getReferralOrders, getReferralOrderStats }
