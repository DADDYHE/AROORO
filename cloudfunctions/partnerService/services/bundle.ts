...
export async function getPartnerIncomeBundle(
  event: BffEvent,
  context: BffContext,
  auth: BffAuth
): Promise<unknown> {
  ...
  const [overviewRes, walletRes, ratesRes, payeeRes, detailsRes] = await Promise.all([
    ...
    safe(
      walletHandlers.getMyIncomeDetails(
        { ...event, type: 'all', page: 1, pageSize },
        context,
        auth
      ),
      'getMyIncomeDetails'
  ...
  return handleSuccess(
    {
      ...
      payee: (payeeRes?.data as { payee?: unknown } | undefined)?.payee || null,
...
export async function getServiceIncomeBundle(
  event: BffEvent,
  context: BffContext,
  auth: BffAuth
): Promise<unknown> {
  ...
  const [overviewRes, walletRes, payeeRes, detailsRes] = await Promise.all([
    ...
    safe(
      walletHandlers.getMyWallet({ ...event, walletType: 'serviceIncome' }, context, auth),
      'getMyWallet'
    ),
    safe(walletHandlers.getMyPayeeAccounts(event, context, auth), 'getMyPayeeAccounts'),
    safe(
      incomeHandlers.getServiceIncomeDetails(
        { ...event, type: 'all', page: 1, pageSize },
        context,
        auth
      ),
...
export async function getReferralBundle(
  event: BffEvent,
  context: BffContext,
  auth: BffAuth
): Promise<unknown> {
  ...
  const [usersRes, orderStatsRes, referralStatsRes] = await Promise.all([
    safe(
      referralHandlers.getMyInvitedUsers({ ...event, page: 1, pageSize }, context, auth),
      'getMyInvitedUsers'
    ),
    safe(
      referralHandlers.getReferralOrderStats({ ...event, type: 'all' }, context, auth),
