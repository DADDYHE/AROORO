 ...
 */
async function validateMallCoupon(openid, couponId, orderAmount, items = []) {
...
async function ensureMallBuyer(openid, inviterId) {
    if (!openid) {
        return;
    }
    try {
        ...
        console.warn('[mallService] ensureMallBuyer 建号失败（不影响下单）:', e?.message);
    }
}
...
async function createOrder(event, _context, auth) {
    ...
    await ensureMallBuyer(openid, event.inviterId);
    ...
    return withRetryableTransaction(async (transaction) => {
        ...
        if (couponId) {
            const lockResult = await validateMallCoupon(openid, couponId, grossAmount, [productId, skuId].filter(Boolean));
...
async function createMultiOrder(event, _context, auth) {
    ...
    return withRetryableTransaction(async (transaction) => {
        ...
        if (couponId) {
            ...
            const cartItemIds = [...new Set(normItems
                    .flatMap((it) => [it.productId, it.skuId].filter(Boolean)))];
            const lockResult = await validateMallCoupon(openid, couponId, totalAmount, cartItemIds);
            validatedCouponDiscount = lockResult.discount;
        }
        else if (couponDiscountNum > 0) {
            throw err('INVALID_PARAMS', '未选择优惠券时不允许折扣');
