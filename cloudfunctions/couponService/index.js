...
const VALID_COUPON_STATUSES = ['unused', 'locked', 'used', 'expired', 'revoked'];
/**
...
async function claimCoupon(event, _context, auth) {
    ...
    if (template.validFrom && startTime > now) {
        throw err('BUSINESS_ERROR', '该优惠券尚未生效');
    }
    let endTime;
...
async function getPopupCoupon(event, _context, auth) {
    ...
    const activeTemplates = templates.data.filter((t) => {
        if (t.validFrom) {
    ...
    const templateIds = activeTemplates.map((t) => t._id || '').filter(Boolean);
    const claimedRes = await db.collection('user_coupons')
        .where({ templateId: _.in(templateIds), ownerId: auth.openid, status: 'unused' })
        .get();
