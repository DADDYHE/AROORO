...
class AuthService {
  ...
  async tryRestoreSession(app) {
    app = app || getApp()
    if (!app || !app.globalData) {
      console.warn('[AuthService] App 实例未就绪，跳过会话恢复')
      return false
    }
  ...
  async login(options = {}) {
    const app = getApp()
    if (!app || !app.globalData) {
      console.warn('[AuthService] App 实例未就绪，无法登录')
      return { success: false, message: '应用未就绪' }
    }
  ...
  async logout() {
    const app = getApp()
    if (!app || !app.globalData) {
      console.warn('[AuthService] App 实例未就绪，无法退出')
      return { success: false, message: '应用未就绪' }
    }
