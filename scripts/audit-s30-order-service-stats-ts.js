 ...
 *   5. scripts/build-all-services.js 包含 stats.js
...
check('build-all-services.js 包含 stats.js', /stats\.js/.test(buildScript || ''))
...
if (pkg) {
  try {
    ...
    check('package.json ci:check 包含 audit:s30-order-service-stats-ts:strict',
      /(?:audit:s30-order-service-stats-ts:strict|audit:all:strict)/.test(cfg.scripts['ci:check'] || ''))
  } catch (e) {
...
//   Sprint 32: payment.ts 已废弃移除（允许作为占位标记保留），迁移完成度 2/2（orders.ts / stats.ts）
...
check('orderService TypeScript 迁移完成（2/2，payment.ts 已废弃或为占位）',
  fs.existsSync(ordersTs) && fs.existsSync(statsTs))
