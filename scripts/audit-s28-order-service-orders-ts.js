 ...
 *   5. scripts/build-order-service.js 存在
...
check('build-order-service.js 包含 orders.js', /orders\.js/.test(buildScript || ''))
...
if (pkg) {
  try {
    ...
    check('package.json ci:check 包含 audit:s28-order-service-orders-ts:strict',
      /audit:s28-order-service-orders-ts:strict/.test(cfg.scripts['ci:check'] || ''))
  } catch (e) {
...
//   - payment.ts 在 Sprint 29 已迁移，stats.ts 在 Sprint 30 已迁移
...
check('payment.ts 已迁移（Sprint 29 完成）', fs.existsSync(paymentTs))
check('payment.js 编译产物存在', fs.existsSync(paymentJs))
