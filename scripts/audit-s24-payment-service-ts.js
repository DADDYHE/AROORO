 ...
 *   5. scripts/build-payment-service.js 存在
 *   6. scripts/build-payment-service.js TARGETS 包含 refund.js
 *   7. package.json 注册 build:payment-service / typecheck:paymentService / audit:s24-payment-service-ts
...
check('build-payment-service.js TARGETS 包含 refund.js', /refund\.js/.test(buildScript || ''))
check('build-payment-service.js 调用 tsc -p tsconfig.paymentService.json', /tsconfig\.paymentService\.json/.test(buildScript || ''))
...
if (pkg) {
  try {
    ...
    pkgOk = true
    check('package.json 注册 build:payment-service', Boolean(cfg.scripts['build:payment-service']))
    ...
    check('package.json build:all 包含 build:payment-service', /build:payment-service/.test(cfg.scripts['build:all'] || ''))
    check('package.json ci:check 包含 audit:s24-payment-service-ts:strict', /audit:s24-payment-service-ts:strict/.test(cfg.scripts['ci:check'] || ''))
  } catch (e) {
