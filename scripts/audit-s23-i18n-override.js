...
// 兼容三种形式：字面量 .limit(200) / .limit(FETCH_LIMIT) + 常量 / .limit(exports.FETCH_LIMIT) + 编译输出常量 = 200
const limitLiteralOk = /\.limit\(\s*200\s*\)/.test(cfCode || '')
const limitConstOk = /\.limit\(\s*FETCH_LIMIT\s*\)/.test(cfCode || '') && /FETCH_LIMIT\s*=\s*200\b/.test(cfCode || '')
const limitExportsConstOk = /\.limit\(\s*exports\.FETCH_LIMIT\s*\)/.test(cfCode || '') && /exports\.FETCH_LIMIT\s*=\s*200\b/.test(cfCode || '')
const limitDetail = !limitLiteralOk && limitConstOk ? '使用 FETCH_LIMIT 常量 = 200' : (!limitLiteralOk && !limitConstOk && limitExportsConstOk ? '使用 exports.FETCH_LIMIT 编译常量 = 200' : '')
check('i18nOverride cloud function 限制 limit=200', limitLiteralOk || limitConstOk || limitExportsConstOk, limitDetail)
