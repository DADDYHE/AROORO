...
/**
 * Sprint 47: paymentService/index TypeScript 迁移审计
 *
 * 检查项：
 *   1. cloudfunctions/paymentService/index.ts 存在
 *   2. cloudfunctions/paymentService/index.js（构建产物）存在
 *   3. tsconfig.paymentService.json include 包含 index.ts
 *   4. scripts/build-all-services.js 包含 index.js target
 *   5. package.json 注册 audit:s47-payment-service-index-ts + strict
 *   6. ci:check 包含 audit:s47-payment-service-index-ts:strict
 *   7. index.ts 强类型化 6 个 handler + 1 个 notify 入口
 *   8. index.ts 抽离 NO_AUTH_ACTIONS / SUPPORTED_ACTIONS 常量
 *   9. index.ts 含 isHttpRequest 工具函数
 *  10. index.ts 含 Runtime shim 修复 CommonJS 导出
 *  11. 严格模式：tsc --noEmit 无错误
 *  12. 严格模式：index.js 头部含 eslint-disable 标记
 *  13. 严格模式：index.js 包含 _mod.exports shim
 *  14. jest 测试 payment-service-index-ts-migration.test.js 存在
 *
 * 退出码：
 *   0 = 全部通过
 *   1 = 至少 1 项不通过
...
const CFG = 'tsconfig.paymentService.json'
...
check('paymentService/index.ts 存在', fs.existsSync(TS))
check('paymentService/index.js（构建产物）存在', fs.existsSync(JS))
...
if (buildScript) {
  ...
  check('build-all-services.js 包含 index.js target', /index\.js/.test(buildScript))
}
...
if (pkg) {
  try {
    ...
    pkgOk = true
    check('package.json 注册 audit:s47-payment-service-index-ts',
      Boolean(cfg.scripts['audit:s47-payment-service-index-ts']))
    check('package.json 注册 audit:s47-payment-service-index-ts:strict',
      Boolean(cfg.scripts['audit:s47-payment-service-index-ts:strict']))
...
if (tsCode) {
  ...
  check('index.ts 含 AuthLike / CloudEvent / CloudContext 接口',
    ...
    && /(?:export\s+interface\s+CloudEvent\b|export\s+type\s+CloudEvent\b)/.test(tsCode)
    && /export\s+interface\s+CloudContext\b/.test(tsCode))
  check('index.ts 含 NO_AUTH_ACTIONS 常量',
    /export\s+const\s+NO_AUTH_ACTIONS/.test(tsCode))
  check('index.ts NO_AUTH_ACTIONS 包含 paymentNotify',
    /NO_AUTH_ACTIONS[\s\S]*?['"]paymentNotify['"]/.test(tsCode))
  check('index.ts 含 SUPPORTED_ACTIONS 常量',
    /export\s+const\s+SUPPORTED_ACTIONS/.test(tsCode))
  check('index.ts SUPPORTED_ACTIONS 包含 6+ 个 action',
    (tsCode.match(/['"](?:createPayment|queryPayment|closePayment|confirmPayment|createRefund|queryRefund|paymentNotify)['"]/g) || []).length >= 6)
  check('index.ts 含 isHttpRequest 工具函数',
    /export\s+function\s+isHttpRequest\b/.test(tsCode))
  check('index.ts 含 isHttpRequest 头/body 判定',
    /event\.headers/.test(tsCode) && /event\.body/.test(tsCode))
...
// 6. 测试存在
const migrationTest = path.join(ROOT, 'test', 'payment-service-index-ts-migration.test.js')
check('测试 payment-service-index-ts-migration.test.js 存在', fs.existsSync(migrationTest))
