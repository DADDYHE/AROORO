 ...
 *   7. scripts/build-all-services.js TARGETS 含 rate-limit-store.js
...
for (const svc of SERVICES_WITH_RATELIMIT) {
  ...
  if (code && /(initGlobalRateLimitFromDb|bootstrapRateLimit)\s*\(/.test(code)) {
    injectedServices.push(svc.name)
  }
  check(
    `${svc.name}/index.js 注入了 initGlobalRateLimitFromDb`,
    code && /(initGlobalRateLimitFromDb|bootstrapRateLimit)\s*\(/.test(code),
    code ? '已注入' : '未注入'
  )
}
...
// 7. scripts/build-all-services.js TARGETS 含 rate-limit-store.js (Sprint 48 合并后)
const buildCommon = readSafe(path.join(ROOT, 'scripts', 'build-all-services.js'))
check(
  'scripts/build-all-services.js TARGETS 含 rate-limit-store.js',
