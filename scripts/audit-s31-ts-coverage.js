    ...
    if (entry.isDirectory()) {
      ...
      if (entry.name.endsWith('.ts')) {
        ...
        if (existing) {
          ...
          results.push({ name: baseName, dir: dir, hasTs: true, hasJs: false, isMigrated: true })
        }
        ...
        if (existing) {
          ...
          results.push({ name: baseName, dir: dir, hasTs: false, hasJs: true, isMigrated: false })
        }
      }
    }
  }
...
// 4. 列出已迁移的 .ts 文件清单
console.log('\n=== 已迁移 .ts 模块清单 ===')
migratedModules.sort((a, b) => {
  return path.relative(ROOT, path.join(a.dir, a.name + '.ts'))
    .localeCompare(path.relative(ROOT, path.join(b.dir, b.name + '.ts')))
}).forEach(m => {
