...
/**
 * Sprint 46: tuanExpiryCheck TypeScript 迁移审计
 */
...
const SVC = 'tuanExpiryCheck'
const CFG = 'tsconfig.tuanExpiryCheck.json'
const NAME = 'tuan-expiry-check'
...
if (code) {
  ...
  check('TuanStatus 联合', /TuanStatus/.test(code))
  check('TuanDealDoc', /TuanDealDoc/.test(code))
  check('COLLECTION = tuan_deals', /COLLECTION\s*=\s*['"]tuan_deals['"]/.test(code))
  check('TARGET_STATUSES 含 published + active', /['"]published['"]/.test(code) && /['"]active['"]/.test(code))
  check('NEW_STATUS = ended', /NEW_STATUS:.*=\s*['"]ended['"]/.test(code))
  check('where status in + endTime<now', /_\.in\(\[\.\.\.TARGET_STATUSES\]\)/.test(code) && /_\.lt\(now\)/.test(code))
  check('update to ended', /status:\s*NEW_STATUS/.test(code))
