 ...
 *   6. scripts/build-common.js TARGETS 含 rate-limit-store.js
 *
 * 退出码：
 *   0 = 全部通过
 *   1 = 至少 1 项不通过
...
check(
  'orderService/index.js 调用 initGlobalRateLimitFromDb',
  /initGlobalRateLimitFromDb\s*\(/.test(orderIdx || '')
)
check(
  'paymentService/index.js 调用 initGlobalRateLimitFromDb',
  /initGlobalRateLimitFromDb\s*\(/.test(paymentIdx || '')
)
...
check('rateLimitCleanup/package.json 存在', !!cleanupPkg)
...
// 6. build-common.js TARGETS
const buildJs = readSafe(path.join(ROOT, 'scripts', 'build-common.js'))
check('build-common.js TARGETS 含 rate-limit-store.js', /rate-limit-store\.js/.test(buildJs || ''))
