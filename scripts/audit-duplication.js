...
const KNOWN_DUPLICATES = [
  ...
  // Sprint 22 之后：utils/BookingDataService.js 与 subpackages/pet/BookingDataService.js
  // 已合并（合并后只保留 utils/ 副本），该对已不存在，本节不再列入。
]
// ALLOWED_DUPLICATES：部署约束导致的有意重复，不计入违规但仍报告位置
const ALLOWED_DUPLICATES = [
  // 云函数部署约束：每个 service 必须自带 common/ 目录，
  // 因此 cloudfunctions/<service>/common/ 与 cloudfunctions/common/ 副本文件
  // 内容一致属预期行为，扫描时由 isCommonCopy() 跳过。
  // 这里仅列出与 common/ 副本无关的特例：
  //（当前无）
]
...
if (ALLOWED_DUPLICATES.length > 0) {
  console.log('\n=== 豁免重复（设计模式）===')
  for (const [a, b] of ALLOWED_DUPLICATES) {
    console.log(`✓ 豁免：${a} <-> ${b}`)
  }
}
...
// Sprint 22 设计：每个云函数 service 必须自带 common/ 目录副本（部署约束），
// 副本文件一致属预期行为，扫描时跳过 cloudfunctions/*/common/*.js
...
for (const dir of TARGET_DIRS) {
  ...
  for (const file of files) {
    ...
    hashMap.get(basename).set(hash, (hashMap.get(basename).get(hash) || []).concat(rel))
  }
}
