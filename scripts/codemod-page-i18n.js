...
function transformOneShowModal(content, startIdx) {
  ...
  const parts = []
  if (titleKey) parts.push(`titleKey: '${titleKey}'`)
  else if (titleStr) parts.push(`title: ${JSON.stringify(titleStr)}`)  // 兜底

  if (contentKey) parts.push(`contentKey: '${contentKey}'`)
  else if (contentStr) parts.push(`content: ${JSON.stringify(contentStr)}`)
