...
class AppStartupOptimizer {
  ...
  async executeDeferredTasks(strategy = 'parallel') {
    ...
    for (const [, tasks] of Object.entries(groups)) {
      if (strategy === 'parallel') {
        const promises = tasks.map(({ task, delay }) => new Promise(resolve => {
          setTimeout(() => {
            task().catch(error => {
  ...
  async _initializeMinimalCoreModules(app) {
    // 关键阶段只初始化首页渲染必需的最小数据集，避免阻塞首屏：
    // 1. 同步设置 globalData 的应用元信息（版本号、构建号）
    // 2. 校验本地存储中关键缓存键的合法性，清理过期数据
    // 3. 预热常用工具类的引入（避免首屏调用时再 require）
    try {
      if (app && app.globalData) {
        const accountInfo = wx.getAccountInfoSync && wx.getAccountInfoSync()
        if (accountInfo && accountInfo.miniprogram) {
          app.globalData.appVersion = accountInfo.miniprogram.version || '1.0.0'
        }
      }
      ...
      // 关键阶段失败不阻塞首屏，仅记录
      console.warn('[StartupOptimizer] 最小化核心模块初始化异常（已忽略）:', error && error.message)
    }
  }
