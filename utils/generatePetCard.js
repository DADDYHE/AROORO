...
class PetCardGenerator {
  ...
  static drawPetInfo(ctx, petData) {
    ...
    ctx.textAlign = 'center'
    ctx.fillText(petData.type === 'dog' ? '狗狗' : '猫咪', 150, startY + lineHeight)
    ...
    ctx.fillText(`年龄: ${petData.age}岁`, 50, infoY)
    ctx.fillText(`体重: ${petData.weight}kg`, 50, infoY + lineHeight)
    ctx.fillText(`品种: ${petData.breed}`, 50, infoY + lineHeight * 2)
    ctx.fillText(`绝育: ${petData.isSterilized ? '是' : '否'}`, 50, infoY + lineHeight * 3)
    ctx.fillText(`疫苗: ${petData.isVaccinated ? '是' : '否'}`, 50, infoY + lineHeight * 4)
    ctx.fillText(`健康: ${petData.healthStatus || '未填写'}`, 50, infoY + lineHeight * 5)
    ctx.fillText(`过敏: ${petData.allergies || '未填写'}`, 50, infoY + lineHeight * 6)
    ctx.fillText(`需求: ${petData.specialNeeds || '未填写'}`, 50, infoY + lineHeight * 7)
  }
