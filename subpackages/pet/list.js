...
Page({
  ...
  deleteSelectedPets() {
    ...
    this.showModal({ titleKey: 'BIZ_FROTRU' })
  },
  ...
  onShareAppMessage() {
    ...
    return {
      title: '我的宠物',
      path: inviterId ? `/subpackages/pet/list?inviterId=${inviterId}` : '/subpackages/pet/list'
    }
  },
  ...
  onBackPress(options) {
    if (this.data.fromCreate) {
      ...
      wx.navigateBack({
        delta: delta,
        fail: (error) => {
          console.error('[APP] wx.navigateBack 失败:', error)
          wx.switchTab({
            url: '/pages/home/index',
            fail: (error) => {
              console.error('[APP] wx.switchTab失败:', error)
            }
          })
        }
      })
