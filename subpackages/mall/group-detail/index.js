...
Page({
  ...
  onShareAppMessage() {
    ...
    const basePath = `/subpackages/mall/group-detail/index?dealId=${this.data.dealId}`
    return {
  ...
  onPreviewSkuThumb(e) {
    ...
    const images = (product && product.images) || [url]
  ...
  onPopupSelectSpec(e) {
    ...
    this.setData({ selectedSpecs: newSpecs, popupSpecGroups: updatedGroups })
    this._updateDisabledValues()
    this._updateSkuPopupState()
  },
  ...
  onQuantityMinus() {
    ...
    this.setData({ quantity, totalPrice: this._calcTotalPrice(quantity) })
  },
  ...
  _calcTotalPrice(quantity) {
    ...
    const qty = quantity !== undefined ? quantity : this.data.quantity
    return (price * qty).toFixed(2)
  },

  _updateTotalPrice(quantity) {
    const product = this.data.products[this.data.selectedProductIndex]
    if (!product) {
