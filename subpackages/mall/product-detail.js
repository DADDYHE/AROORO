...
Page({
  ...
  data: {
    ...
    iconShare: 'cloud://cloudbase-d7getcjqy33b13475.636c-cloudbase-d7getcjqy33b13475-1433773870/icons/分享.svg',
    iconService: 'cloud://cloudbase-d7getcjqy33b13475.636c-cloudbase-d7getcjqy33b13475-1433773870/icons/客服.svg',
  ...
  _updateDisabledValues() {
    ...
    const enabledSkus = product.skus.filter(s => s.enabled !== false && s.stock > 0)
    const updatedGroups = popupSpecGroups.map((group, groupIndex) => {
      ...
      group.values.forEach(val => {
        if (hasOtherSelections) {
          ...
          const hasMatch = enabledSkus.some(sku => {
            if (sku.specIds) {
              return Object.keys(candidate).every(specId => candidate[specId] === '' || sku.specIds[specId] === candidate[specId])
            }
  ...
  onChangeQuantity(e) {
    ...
    let { quantity, selectedSkuId, product } = this.data
  ...
  onSkuConfirm() {
    ...
    if (skuPopupMode === 'cart') {
      ...
      let url = `/subpackages/mall/order-confirm?productId=${product._id}&quantity=${quantity}&skuId=${selectedSkuId}`
      wx.navigateTo({ url })
    }
  },
  ...
  onBuyNow() {
    ...
    if (product.skuType === 'multi' && product.skus && product.skus.length > 0) {
      ...
      let url = `/subpackages/mall/order-confirm?productId=${product._id}&quantity=${this.data.quantity}`
      wx.navigateTo({ url })
    }
  },
